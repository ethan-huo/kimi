...
运行时数据（sessions、telemetry、config 等）放在 `~/.kimi-code`（上游默认），安装目录放在 `~/.kimi/code`；升级成功后会自动清理旧备份。运行时:`kimi` launcher 快路径 `exec bun dist/main.mjs`,并置 `KIMI_CODE_NO_AUTO_UPDATE=1`
...
mkdir -p ~/.kimi/code && curl -fsSL "https://github.com/ethan-huo/kimi/releases/download/v$VER/kimi-darwin-arm64.tar.gz" | tar -xz -C ~/.kimi/code
ln -sf ~/.kimi/code/kimi ~/.local/bin/kimi   # 确保 ~/.local/bin 在 PATH
kimi --version
```
